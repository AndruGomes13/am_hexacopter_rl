...
import IPython
from environments.hexcopter_t_yaw.action_models import (
...
from environments.hexcopter_t_yaw.config import DomainRandomizationConfig, EnvConfig, FloatRange
from environments.hexcopter_t_yaw.observation_models import (
    Observation,
    ActorObservationType,
    actor_observation_model_factory,
    critic_observation_model_factory,
)
from environments.hexcopter_t_yaw.env_utils import get_env_xml_path, init_trajectory, sample_trajectory
...
class Hexcopter3DEnv(PipelineEnv):
    ...
    def __init__(self, config: Optional[EnvConfig] = None, debug=False, **kwargs):
        ...
        self.actor_observation_model.resolve_fields(action_size=self.action_model.get_action_dim(), action_history_len=self.config.action_history_len, sensor_data_history_len=self.config.sensor_data_history_len, action_buffer_len=self.config.action_delay_discrete)
        self.critic_observation_model = critic_observation_model_factory(self.config.critic_observation_model)
        self.critic_observation_model.resolve_fields(action_size=self.action_model.get_action_dim(), action_history_len=self.config.action_history_len, sensor_data_history_len=self.config.sensor_data_history_len, action_buffer_len=self.config.action_delay_discrete)
    ...
    def step(self, state: AugmentedEnvState, action: jp.ndarray) -> AugmentedEnvState:
        ...
        # --- Un-Normalize Action ---
        # --- Action delay ---
        action_to_apply = prev_pipeline_state_aug.get_n_action_history(self.config.action_delay_discrete - 1)[:6] if self.config.action_delay_discrete > 0 else action[:6]

        # --- Action Dynamics ---
        ctrl = self.action_model.mujoco_ctrl_from_normalized_action(action_to_apply, prev_pipeline_state_aug)
        # Clamp the joint 
    ...
    def reward(self, reward_args: RewardArgs):
        ...
        cumulative_reward = (
            ...
            + reward_end_effector_dist_to_target
        )
        ...
        reward_metrics = RewardMetrics(
            ...
            end_effector_dist_to_target=reward_end_effector_dist_to_target
        )
    ...
    def reset_with_progress(self, rng: jp.ndarray, curriculum_info: CurriculumProgressInfo):
        ...
        rng_drone_state, rng_domain_rand, rng_arm_state, traj_key, rng_state = jax.random.split(rng, 5)
        ...
        # --- Set the arm state ---
        drone_arm_1_pitch,drone_arm_2_pitch  = self._random_arm_pitch(rng_arm_state)
        drone_arm_1_joint = drone_arm_1_pitch
        drone_arm_2_joint = drone_arm_1_pitch + drone_arm_2_pitch
        qpos, qvel = set_arm_state(qpos, qvel, drone_arm_1_joint, drone_arm_2_joint)        
        ...
        last_ctrl = jp.array([0,0,0,0,drone_arm_1_joint, drone_arm_2_joint])
        ...
        trajectory = init_trajectory(traj_key, 10, 60)
        ...
        augmented_pipeline_state = AugmentedPipelineState(
            ...
            action_history=jp.zeros((self.config.action_buffer_size, self.mj_model.nu)),
    ...
    def _reward_end_effector_dist_to_target(self, reward_args: RewardArgs, p: jp.ndarray, weight_type: Literal["squared", "exp"] = "squared"):
        ...
        if weight_type == "squared":
            ...
            return -self.config.stage_config.reward_end_effector_dist_to_target(p) * (jp.exp(-jp.linalg.norm(next_sensor_data.end_effector_position - goal_pos)) - 1)
    ...
    def _random_drone_state_with_progress(self, rng: jp.ndarray, p) -> DroneState:
        ...
        yaw = jax.random.uniform(keys[0], minval=0.0, maxval=2 * jp.pi)

        # --- Yaw velocity
        max_yaw_rate = self.config.stage_config.reset_drone_ang_vel_rand(p)
        ...
        return DroneState(pos, vel, yaw, yaw_rate)

    # --- Arm
    def _random_arm_pitch(self, rng: jp.ndarray) -> Tuple[float, float]:
        key_pitch = jax.random.split(rng, 2)
        drone_arm_1_pitch = jax.random.uniform(key_pitch[0], minval=0, maxval=jp.deg2rad(180))
        drone_arm_2_pitch = jax.random.uniform(key_pitch[1], minval=-jp.deg2rad(10), maxval=jp.deg2rad(45))
        drone_arm_2_pitch = jp.clip(drone_arm_2_pitch, -drone_arm_1_pitch)  # Ensure it is within limits
        return drone_arm_1_pitch, drone_arm_2_pitch
